...
function csv_edihist_log ( $msg_str ) {
	//$logfile = dirname(__FILE__) . "/edi_history_log.txt";
	$logfile = $GLOBALS['OE_EDIH_DIR'] . "/edi_history_log.txt";
	$rslt = FALSE;
...
function csv_log_html() {
	...
	//$fp = dirname(__FILE__) . "/edi_history_log.txt"; 
    $fp = $GLOBALS['OE_EDIH_DIR'] . "/edi_history_log.txt"; 
	$fh = fopen( $fp, 'r');
	if ($fh !== FALSE) {
...
function csv_log_archive() {
	$dte = date('Ymds');
	//$zname = dirname(__FILE__).DIRECTORY_SEPARATOR.$dte.'_edihistory_log.txt';
	//$logname = dirname(__FILE__).DIRECTORY_SEPARATOR.'edi_history_log.txt';
	//$archname = dirname(__FILE__).DIRECTORY_SEPARATOR.'edihistory_archive.zip';
	$str_out = '';
    $zname = $GLOBALS['OE_EDIH_DIR'].DIRECTORY_SEPARATOR.$dte.'_edihistory_log.txt';
    $logname = $GLOBALS['OE_EDIH_DIR'].DIRECTORY_SEPARATOR.'edi_history_log.txt';
    $archname = $GLOBALS['OE_EDIH_DIR'].DIRECTORY_SEPARATOR.'edihistory_archive.zip';
	//
	$fs = filesize($logname);
	if ($fs == false && $fs < 512) {
...
function csv_setup(&$out_str) {
    ...
    if ($edihist_dir) {
       ...
       $out_str .= "setup: failed to obrain OpenEMR Site directory";
       return false;
    }
	//
	...
	if ($isOK) { 
		...
		if (!mkdir($edihist_dir, 0755, true)) {
			...
			$isOK = FALSE;
			die("Failed to create folder...$edihist_dir");
		} else { 
			...
			if (!mkdir($csv_dir, 0755, true) ) {
				...
				die("Failed to create csv folder...$csv_dir");
			}
			if (!mkdir($archive_dir, 0755, true) ) {
				$isOK = FALSE;
				csv_edihist_log("Setup: Failed to create archive folder...$archive_dir");
				die("Failed to create archive folder...$archive_dir");
			}
			//
			foreach ($p_ar as $key=>$val) {
				// make the storage subdirs
				//
				//$type_dir = $dp.DIRECTORY_SEPARATOR.$p_ar[$key]['type']; 
				...
				if (!is_dir($type_dir) && !mkdir($type_dir, 0755, false) ) {
					...
					die("Failed to create storage directory $key");
				}
				...
				if (is_array($hdr_f) ) {
					...
					if ($fpath) {
						...
						//$fh = fopen($basedir . DIRECTORY_SEPARATOR . $fpath, 'x');
                        $fh = fopen($fpath, 'x');
						if ($fh !== FALSE) {
				...
				if (is_array($hdr_c) ) {
					...
					if ($cpath) {
						...
						//$fh = fopen($basedir . DIRECTORY_SEPARATOR . $cpath, 'x');
                        $fh = fopen($cpath, 'x');
						if ($fh !== FALSE) {
					...
					$isOK = FALSE;
					csv_edihist_log("Did not get header row for $key claim");
				}
				//			
			}
		}
...
function csv_parameters($type="ALL") {
	// 
	// This will need the OpenEMR 'oe_site_dir' to replace IBR_HISTORY_DIR
	// global 
    ...
    /*
	$p_ar['batch'] = array("type"=>'batch', "directory"=>IBR_HISTORY_DIR.'/batch', "claims_csv"=>IBR_HISTORY_DIR.'/csv/claims_batch.csv', 
						"files_csv"=>IBR_HISTORY_DIR.'/csv/files_batch.csv', "datecolumn"=>'0', "fncolumn"=>'1', "regex"=>'/\-batch(.*)\.txt$/');	
	//	
	//$p_ar['csv'] = array("type"=>'csv', "directory"=>IBR_HISTORY_DIR . '/csv', "claims_csv"=>'ibr_parameters.csv',  
	//					"files_csv"=>'', "column"=>'', "regex"=>'/\.csv$/'); 						
	$p_ar['ibr'] = array("type"=>'ibr', "directory"=>IBR_HISTORY_DIR.'/ibr', "claims_csv"=>IBR_HISTORY_DIR.'/csv/claims_ibr.csv', 
						"files_csv"=>IBR_HISTORY_DIR.'/csv/files_ibr.csv', "datecolumn"=>'0', "fncolumn"=>'1', "regex"=>'/\.ibr$/');
	$p_ar['ebr'] = array("type"=>'ebr', "directory"=>IBR_HISTORY_DIR.'/ebr', "claims_csv"=>IBR_HISTORY_DIR.'/csv/claims_ebr.csv', 
						"files_csv"=>IBR_HISTORY_DIR.'/csv/files_ebr.csv', "datecolumn"=>'0', "fncolumn"=>'1', "regex"=>'/\.ebr$/');
	$p_ar['dpr'] = array("type"=>'dpr', "directory"=>IBR_HISTORY_DIR.'/dpr', "claims_csv"=>IBR_HISTORY_DIR.'/csv/claims_dpr.csv', 
						"files_csv"=>'', "datecolumn"=>'1', "fncolumn"=>'5', "regex"=>'/\.dpr$/');						
	$p_ar['f997'] = array("type"=>'f997', "directory"=>IBR_HISTORY_DIR.'/f997', "claims_csv"=>IBR_HISTORY_DIR.'/csv/claims_997.csv', 
						"files_csv"=>IBR_HISTORY_DIR.'/csv/files_997.csv', "datecolumn"=>'0', "fncolumn"=>'1', "regex"=>'/\.99[79]$/');
	$p_ar['f277'] = array("type"=>'f277', "directory"=>IBR_HISTORY_DIR . '/f277', "claims_csv"=>IBR_HISTORY_DIR.'/csv/claims_277.csv', 
						"files_csv"=>IBR_HISTORY_DIR.'/csv/files_277.csv', "datecolumn"=>'0', "fncolumn"=>'1', "regex"=>'/\.277([ei]br)?$/');
	// OpenEMR stores era files, but the naming scheme is confusing, so we will just use our own directory for them 
	$p_ar['era'] = array("type"=>'era', "directory"=>IBR_HISTORY_DIR.'/era', "claims_csv"=>IBR_HISTORY_DIR.'/csv/claims_era.csv', 
						"files_csv"=>IBR_HISTORY_DIR.'/csv/files_era.csv', "datecolumn"=>'0', "fncolumn"=>'1', "regex"=>'/835[0-9]{5}\.835*|\.(era|ERA)$/');
	$p_ar['ta1'] = array("type"=>'ta1', "directory"=>IBR_HISTORY_DIR.'/f997', "claims_csv"=>'', 
						"files_csv"=>IBR_HISTORY_DIR.'/csv/files_997.csv', "datecolumn"=>'0', "fncolumn"=>'1', "regex"=>'/\.ta1$/i');
	$p_ar['ack'] = array("type"=>'ack', "directory"=>IBR_HISTORY_DIR.'/f997', "claims_csv"=>'', 
						"files_csv"=>IBR_HISTORY_DIR.'/csv/files_997.csv', "datecolumn"=>'0', "fncolumn"=>'1', "regex"=>'/\.ack$/i');
	$p_ar['text'] = array("type"=>'text', "directory"=>IBR_HISTORY_DIR.'/text', "claims_csv"=>'',
...
function csv_dirfile_list ($type) {
	...
	$dirfiles = array();
	//
	//$dirfiles = scandir($search_dir);   //, SCANDIR_SORT_NONE SCANDIR_SORT_DESCENDING
	//foreach($dirfiles as $ky=>$val) {
	//	if (!preg_match($ext_re, $val) ) { unset($dirfiles[$ky]); }
	//}
	        ...
	        while (($file = readdir($dh)) !== false) {
	            if (is_file($search_dir.$file) ) {
					if (preg_match($ext_re, $file) ) { 
						...
						if (($type == 'ack' || $type == 'ta1') && basename($params['directory']) == 'f997') {
...
function csv_processed_files_list ($type) {
	...
	if ($type == 'dpr') {
		//$csv_file = dirname(__FILE__).$param['claims_csv'];
        $csv_file = $param['claims_csv'];
		$csv_col = '5';
	} else {
		//$csv_file = dirname(__FILE__).$param['files_csv'];
...
function csv_newfile_list($type) {	
	...
	if (! strpos("|era|f997|ibr|ebr|dpr|f277|batch|ack|ta1|text", $type) ) {	
...
function csv_write_record($csv_data, $file_type, $csv_type) {
	...
	if (! strpos("|era|f997|ibr|ebr|dpr|f277|batch|ack", $file_type) ) {	
		csv_edihist_log("csv_writedata_csv error: incorrect file type $file_type");
		return FALSE;
	}
 ...
 * except in ebr, ibr, and dpr files which have something else in pos 4.  Since we are 
 * interested usually in the filename, the number is irrelevant usually, but see the
...
function csv_file_with_pid_enctr ($ptctln, $filetype='batch', $srchtype='encounter' ) { 
	...
	//"|era|835|f997|999|ibr|ebr|dpr|f277|batch|837|ta1|ack"
	if (! strpos(IBR_FTYPES, $filetype) ) {	
	...
	if (count($match2)) {
		...
		// $ptctln has no non-digit characer
		if ($srchtype == 'ptctln') {
...
function csv_file_by_controlnum($type, $control_num) {
	...
	//	
	if (! strpos("|era|f997|f277|batch", $type) ) {	
...
function csv_to_html($file_type, $csv_type, $row_pct = 1, $datestart='', $dateend='') { 
	...
	if (preg_match('/\d{2}\/\d{2}\/\d{4}/', $datestart) && preg_match('/\d{2}\/\d{2}\/\d{4}/', $dateend) ) {
		...
		// or if yyyy/mm/dd $d1 = explode( "/", $datestart); $ds = $d1[0] . $d1[1] . $d1[2];
		...
		$row_pct = 1;
		// debug
		// echo "csv_to_html: $ds $de <br />" .PHP_EOL;
	}
	//
	$f_name	= basename($fp);
		...
		if ($fh !== FALSE) {
			while (($data = fgetcsv($fh, 1024, ",")) !== FALSE) {
				...
				if ($idx == 0) { 
					...
					// debug
					//echo ($isok) ? "isok TRUE "  : "isok FALSE <br />" .PHP_EOL;
					//echo substr($data[$dtcol], 0, 8) . " $ds   $de $csv_type $file_type<br />" .PHP_EOL;
					//
					if ($isok) { $csv_d[] = $data; }	
				}
				$idx++;
	...
	// 
	if ($is_date) {
		$csv_html .= "<p><h4>Table: $f_name &nbsp;&nbsp; Start Date: $datestart &nbsp; End Date: $dateend &nbsp;Rows: $rwct</h4>
	 </p>".PHP_EOL;
	} else {
		$csv_html .= "<p><h4>Table: $f_name &nbsp;&nbsp; Rows: $ln_ct &nbsp;&nbsp; Shown: $rwct</h4>
	 </p>".PHP_EOL;
	}
	//
	 ...
	 if ($csv_type == 'file') {
		 ...
		 if ($file_type == 'era') {
			 ...
			 for ($i=$rwst; $i<$ln_ct; $i++) {
				 ...
				 $csv_html .= "<td>$dt</td>".PHP_EOL;
				 //CLM01
				 $csv_html .= "<td><a class='btclm' target='_blank' href='edi_history_main.php?fvbatch={$csv_d[$i][7]}&btpid=$pidenc'>$pidenc</a></td>".PHP_EOL;
				 //Status
...
function csv_check_filepath($filename, $type = "ALL") {
	...
	//
	// $t_dir should be like /var/www/htdocs/edihistory/edihist
	$goodpath = "";
	$fn = basename($filename);
	//$t_dir = dirname(__FILE__); 
    $t_dir = $GLOBALS['OE_EDIH_DIR'];
	//
	if ($type != "ALL") {
		$p = csv_parameters($type);
		if (is_array($p) && array_key_exists('type', $p) ) {
			// type was found  -- directory is like '/batch'
			//$f_dir = $p['directory'];
			//
			//$fp = $t_dir.$f_dir.DIRECTORY_SEPARATOR.$fn;
			//
...
function csv_verify_file( $file_path, $type, $val_array=FALSE ) {
	...
	if ($fp) {
		...
		if ( $st_pos === FALSE ) { 
		  ...
		  csv_edihist_log ("ibr_verify_file Error, not a valid $type file: $file_path");
...
function csv_x12_segments($file_path, $type, $seg_array = FALSE) {
	// transform an x12 file into an array of segments
	//
	// do verifications 
	...
	$se_str = "SE".$elem_d;
	//
	// unexpected, but this may be off by one, probably depending on whether trailing seg_d is present
	// $ct_off = ($f_str[-1] == $seg_d) ? 0 : 1;
	//$seg_ct = substr_count($f_str, $seg_d);
	//
	$idx = -1;
	$islast = true;
	while ($islast) {
		...
		$islast = strpos($f_str, $seg_d, $seg_end+1); 
		//
		...
		// speed it up by a negative check first
		//if (substr($seg_text, 0, 2) != "ST" && substr($seg_text, 0, 2) != "SE") { continue;}
