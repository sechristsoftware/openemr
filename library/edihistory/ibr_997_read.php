...
function ibr_997_parse($seg_ar) {
	 ...
	 foreach($ar_997_segments as $seg_997 ) {
		...
		if ( $ar_seg[0] == "TA1" ) { 
			...
			if ($bt_file == FALSE) { $bt_file = $ta1ctlnum; }
			//
			continue;
		}
		...
		if ( $ar_seg[0] == "IK5" || $ar_seg[0] == "AK5" ) { 
			...
			if ($ak5status != "A" ) {
				...
				$cml_info = ibr_batch_find_claim_enctr($ak2st02, $bt_file);
...
function ibr_997_akhtml($aksegments, $delims, $btisa13='', $html=true) {
	...
	if ( !is_array($aksegments) || !count($aksegments)) {
		$str_html .= "<p>No rejected claims found in file</p>".PHP_EOL;
		csv_edihist_log("ibr_997_akhtml: No rejected claims or invalid segments array");
		return $str_html;
	}
	
	$btctlnum = ($btisa13) ? $btisa13 : 'unknown';
	$bt_file = ($btisa13) ? ibr_batch_find_file_with_controlnum($btisa13) : 'unknown';
	//
	$elem_d = $delims['e'];
	$sub_d = $delims['s'];
	$isfound = false;
	...
	foreach($aksegments as $segstr) {
		...
		if ($seg[0] == 'AK2') {
			...
			$str_html .= "<p>ST: $batchst <br /> (837 ICN: $btctlnum $bt_file)</p>";
			if ($bt_file && $bt_file != 'unknown') {
				$stinfo = ibr_batch_find_claim_enctr($batchst, $bt_file);
				if (count($stinfo) > 0) {
					$str_html .= "<p>Name: {$stinfo[2]}, {$stinfo[3]} Ctl: {$stinfo[1]}</p>".PHP_EOL;
				}
			}
			continue;
		}
 ...
 * @uses csv_parameters()
 * @param array $file_array -- optional, this array is sent from the ibr_uploads.php script
