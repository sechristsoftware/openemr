 ...
 * 
 * @todo    adapt to only call functions for actually used
 *          file types
 * 
...
function ibr_disp_newfiles() {
	// new files output will be rendered in a popup window
	//
	// alternative here is to get the newfiles array first and
	// pass it as an argument -- would avoid calling function where
	// the user does not have that type of file -- output goes to tab panel
	//
	...
	//"|batch|ack|ta1|f997|f277|ibr|ebr|dpr|era|text"
	$tplist = substr(IBR_FTYPES, 1);	// skip the first character '|'
	$ftype = explode('|', $tplist);
	foreach($ftype as $tp) {
		$upload_ar = csv_newfile_list($tp);
		//$str_html .= "$tp $htm $er " . count($upload_ar) . '<br />'.PHP_EOL;
		if (count($upload_ar) > 0) {
			if ($tp == 'batch') { 
				$str_html .= ibr_batch_process_new($upload_ar, $htm); continue; 
			} elseif ($tp == 'ack') { 
				$str_html .= ibr_ack_process_new($upload_ar, $htm); continue; 
			} elseif ($tp == 'ta1') { 
				$str_html .= ibr_ack_process_new($upload_ar, $htm); continue; 
			} elseif ($tp == 'f997') { 
				$str_html .= ibr_997_process_new($upload_ar, $htm, $er); continue; 
			} elseif ($tp == 'f277') { 
				$str_html .= ibr_277_process_new($upload_ar, $htm, $er); continue; 
			} elseif ($tp == 'ibr') { 
				$str_html .= ibr_ebr_process_new_files($upload_ar, 'ibr', $htm, $er); continue; 
			} elseif ($tp == 'ebr') { 
				$str_html .= ibr_ebr_process_new_files($upload_ar, 'ebr', $htm, $er); continue; 
			} elseif ($tp == 'dpr') { 
				$str_html .= ibr_dpr_process_new($upload_ar, $htm, $er); continue; 
			} elseif ($tp == 'era') { 
				$str_html .= ibr_era_process_new($upload_ar, $htm, $er); continue; 
			} elseif ($tp == 'text') { 
				// do nothing 
				continue; 
			} else {
...
function ibr_disp_claimst() {
	...
	if (isset($_GET['btpid'])) {
		...
		//$clmid = filter_input(INPUT_POST, 'enctrbatch', FILTER_VALIDATE_INT);
	...
	if (strlen($filename) >= 13 && strlen($filename) <= 14) {
		...
		$btname = ibr_batch_find_file_with_controlnum($isa13);
		if (!$btname) {
		...
		$btname = ibr_batch_find_file_with_controlnum($filename);
		if (!$btname) {
			$str_html .= "Failed to identify batch file <br />";
			return $str_html;
		}
		...
		//$enc_ar = ibr_batch_find_file_with_enctr ($clmid);
		$enc_ar = csv_file_with_pid_enctr($clmid, 'batch', 'ptctln');
		if (is_array($enc_ar) && count($enc_ar) ) { 
			foreach($enc_ar as $enc) { 	
				//ibr_batch_get_st_block ($batch_file, $st_clm_num=NULL, $pid_enctr=NULL, $plain_text=FALSE)		
 ...
 * 
 * @uses ibr_era_find_file_with_pid_enctr()
...
function ibr_disp_eraClp() {
	...
	//ibr_era_find_file_with_pid_enctr ($pid_enctr, $srchtype='encounter' ) //[i](pid-enctr, trace, filename)	
	$enc_ar = ($era_enc) ? csv_file_with_pid_enctr($era_enc, 'era', 'encounter') : false;
	//$enc_ar = ($era_enc) ? csv_file_with_pid_enctr($era_enc, 'era', 'encounter')
	// if $enc_ar is an array [i](pid, encounter, filename)	
	// update:  array [i](pid-enc, trace, filename)	
	if (is_array($enc_ar) && count($enc_ar) ) {  		
		foreach($enc_ar as $enc) { 
			//$pe = $enc[0] . "-" . $enc[1];
			$pe = $enc[0];
 ...
 * 
 * @uses ibr_era_find_file_with_pid()
 * @uses ibr_era_find_file_with_enctr()
...
function ibr_disp_era_post() {
	//array
	$str_html = '';
	//'subpid835''subenctr835''subtrace835'
	...
	if ( isset($_POST['subtrace835']) && isset($_POST['trace835']) && $_POST['trace835'] ) {
		...
		$eft = ibr_era_find_file_with_trace($trace);
		// array(file, trace)	
		if (is_array($eft) && count($eft) ) {
			// should only be one
			foreach($eft as $val) { 
				$str_html .= ibr_era_html_page($val[0], $val[1], 0, $search, $val[0]);
			}
		} else {
			$str_html .= "Did not find $trace in data <br />".PHP_EOL;
		}
		//csv_file_with_pid_enctr ($ptctln, $filetype='batch', $srchtype='encounter' ) 
		...
		//$ef = ($pid) ? ibr_era_find_file_with_pid_enctr($pid, $search) : false;	
		...
		//$ef = ($enctr) ? ibr_era_find_file_with_pid_enctr($enctr, $search) : false;
		$ef = ($enctr) ? csv_file_with_pid_enctr ($enctr, 'era', 'encounter') : false;	
	}
	// ibr_era_find_file_with_pid_enctr ($pid_enctr, $srchtype='encounter' )
	//  returns (pid_enctr, trace, filename)
	// multiple files are possible for pid or encounter
	if ($isenc || $ispid) {
 ...
 * 
 * @uses ibr_era_find_file_with_pid_enctr()
 * @uses ibr_era_find_file_with_trace()
...
function ibr_disp_era_get() {
	...
	if (!$fname && $pe && $stype=='encounter') {
		...
		//$ef = ibr_era_find_file_with_pid_enctr($pe, $stype);
		//[i](pid_encounter, number, filename)
...
function ibr_disp_status_resp() {
	...
	if (isset($_GET['pidenc']) && strlen($_GET['pidenc'])) {
		...
		// the rspstnum is the 277 ISA13:ST02
