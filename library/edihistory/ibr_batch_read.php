...
/**
 * get the batch file containing the control_num ISA13
 * 
 * @uses csv_search_record()
 * @param string $control_num   the interchange control number
 * @return  string              the batch file name
 */
function ibr_batch_find_file_with_controlnum($control_num) {
	// get the batch file containing the control_num
	// array('Date', 'FileName', 'Ctn_837', 'claim_ct', 'x12_partner');
	//csv_file_by_controlnum($type, $control_num)
	$ctln = substr($control_num, 0, 9);
	$search = array('s_val'=>$ctln, 's_col'=>2, 'r_cols'=>array(1));
	$result = csv_search_record('batch', 'file', $search, "1");
	if (is_array($result) && count($result[0]) == 1) {
		$batch_file = $result[0][0];
	} else {
		$batch_file = FALSE;
	}
	return $batch_file;
...
/**
 * look in the csv table for the claims and find the encounter and associated filename
 * 
 * @uses csv_search_record() 
 * @param mixed $encounter   the encounter number
 * @return array             (encounter, filename, and ST number) possibly more than one
 */ 
function ibr_batch_find_file_with_enctr ( $encounter ) { 
	// 
	// return the encounter, filename, and ST number, but there may be more than one file, so return an array
	//	
	if (!$encounter) {
		return "invalid encounter data<br />" . PHP_EOL;
	}
	$e = strval($encounter);
	//array('PtName', 'SvcDate', 'clm01', 'InsLevel', 'Fee', 'PtPaid', 'File_837', 'Ctn_837', 'Provider' );
	$search = array('s_val'=>$e, 's_col'=>2, 'r_cols'=>array(2, 6, 7));
	//
	$finfo = csv_search_record('batch', 'claim', $search, "2");
	//
...
/**
 * Segment string transformed into array, with attempt to allow for imperfect selection of segment
 * 
 * @deprecated
 * @see csv_x12_segment_to_array()
 * @param string $sgmt_str   a substring of all or part of a segment 
 * @return array             exploded $sgmt_str by SEG_ELEM_DELIM
 */ 
function ibr_batch_parse_segment($sgmt_str) {
	//
	// truncate segment string to only the segment
	//
	$slen = strlen($sgmt_str);
	if (!$slen) { return FALSE; }
	// find segment delimiters (up to two)
	$p1 = strpos($sgmt_str, SEG_TERM_DELIM);
	$p2 = strpos($sgmt_str, SEG_TERM_DELIM, $p1+1 );
	if ($p1===FALSE) {
		// assume we have just the segment text
		$seg1 = trim($sgmt_str);
	} elseif ($p1 && $p2 && ($p2 > $p1) && ($p2-$p1 < strlen($sgmt_str)) ) {
	...
	} elseif ($p1 !== FALSE && $p1+1 == strlen($sgmt_str) ) {
		// assume we have segment ~
		...
		// no conjecture matched, just return it		
		$seg1 = trim($sgmt_str);
	}
	//
	//echo $sgmt_str . PHP_EOL;
	// echo $seg1 . PHP_EOL;
	$ar_seg = explode (SEG_ELEM_DELIM, $seg1 );
	//
	return $ar_seg;
}

/** 
 * parses an OpenEMR claim batch file for control number and other items
 *  
 * @deprecated
 * @uses csv_x12_segment_to_array()
 * @param string $batch_dir the directory path to the batch file
 * @param array $ar_files an array of filenames in $batch_dir to be parsed
 * @return array    
 */
function ibr_batch_data($batch_dir, $ar_files) {
	// add new entries to batch_files summary data table
	$ar_batch_file = array();
	//
	foreach ($ar_files as $batch) { 
		// full path name
		$batchfullpath = $batch_dir . DIRECTORY_SEPARATOR . $batch;
		//
		$ft = ibr_batch_datefromfilename($batch);
		//
		$ftime = ($ft) ? $ft : date ("Ymd", filemtime($batchfullpath));;
		//
		$bstr = file_get_contents($batchfullpath);
		//
		// debug
		// echo "$batch is " . strlen($bstr ) . " characters" . PHP_EOL;
		//
		//$isa_seg = ibr_batch_parse_segment( substr($bstr, 0, 109) );
		$isa_seg = csv_x12_segment_to_array(substr($bstr, 0, 109), SEG_TERM_DELIM, SEG_ELEM_DELIM);
		//
		$ctl = $isa_seg[13];
		$x12partner = $isa_seg[6];
		$x12version = $isa_seg[12];
		// since each claim is in a ST-SE block, claim count the GE segment ST count
		// We assume only one GS...GE functional group
		// otherwise a gs_num-st_num type field is needed
		$btail = substr($bstr, -30);
		$ge_ct_pos = strpos($btail, SEG_TERM_DELIM . "GE" . SEG_ELEM_DELIM);
		$ge_seg = substr($btail, $ge_ct_pos + 1, 15);
		$ge_ar = csv_x12_segment_to_array($ge_seg);
		$clm_ct = $ge_ar[1];
		// add the values to the array"batch_x12_version" => $x12version,
		$ar_batch_file[] = array("batch_file_time" => $ftime,
		                         //"batch_dir" => $batch_dir,
		                         "batch_file" => $batch, 
		                         "batch_control_num" => $ctl, 
		                         "batch_claim_count" => $clm_ct, 
		                         "batch_x12_partner" => $x12partner,
		                         "batch_x12_version" => $x12version
		                       );
	} // end foreach
	//
	return $ar_batch_file;
}
 ...
 * @uses ibr_batch_find_file_with_enctr()
 * @param string $batch_file     the batch file name
 * @param string $st_clm_num     the ST02 number in the batch file
...
function ibr_batch_get_st_block ($batch_file, $st_clm_num=NULL, $pid_enctr=NULL, $plain_text=FALSE) {
	...
	if ($bfullpath) {
		...
		if ($batch_file && !$bfullpath) {
			$msg_str .=	"file not found: $btchname". PHP_EOL;
		} elseif (!$batch_file) {
			$msg_str .=	"batch file name not supplied";
		}
		if ($pid_enctr) {
			$pe1 = trim($pid_enctr);
			$btnm1 = ibr_batch_find_file_with_enctr ($pe1);
			...
			//
		} elseif ( strlen($st_clm_num) == 13 ) {
			$ctln = substr($st_clm_num, 0, 9);
			$stnum = substr($st_clm_num, -4);
			//$btnm1 = ibr_batch_find_file_with_controlnum($ctln);
 ...
 * @return array
 */
function ibr_batch_find_claim_enctr($st_num, $batchname='', $pidenctr='') { 
	//ibr_batch_find_claim_enctr($ak2st02, $bt_file, $ctx302)
	...
	if (strlen($stnum) == 13) {
		...
		//$btfname = ibr_batch_find_file_with_controlnum($ctln);
	...
	if (strlen($batchname) == 9 && !$btfname) {
		...
		//$btfname = ibr_batch_find_file_with_controlnum($batchname);
	...
	// pregmatch($seg_st, $bstr, $matches, PREG_OFFSET_CAPTURE, 0)
	// $st_pos = (count($matches)) ? $matches[1] : false;
	//
	// get the ending of the ST--SE block so we can take this as a substring
	// $seg_se '/SE'.SEG_ELEM_DELIM.'\d{2,3}'.SEG_ELEM_DELIM.$stnum
	// pregmatch($seg_se, $bstr, $matches, PREG_OFFSET_CAPTURE, $st_pos)
	// $se_pos = (count($matches)) ? $matches[1] : false;
	// if ($se_pos) { $se_pos = $se_pos + strlen($matches[0]; }
	$se_pos = strpos($bstr, $seg_se, $st_pos);
	$se_pos2 = strpos($bstr, $seg_st2, $se_pos);
	$se_pos3 = strpos($bstr, SEG_TERM_DELIM, $se_pos2);
	// now $se_pos should be 6 less than $se_pos2 so the end of the block should be $se_pos2 + 5
	// $seg_block = substr($bstr, $st_pos, $se_pos2+5)
	//
	// if patient is subscriber, then patient is in NM1*IL*, which comes after HL*2
	// otherwise (probably), find the HL*3 and PAT* segment and patient will be in NM1*QC*
	// these SBR*P* give the possibilities that are in the OpenEMR Claim.class.php.
	// However, a possible error is when the names are omitted from the claim in the batch
...
function ibr_batch_csv_data($batch_file_path) {
	...
	if ( is_array($ar_batch) ) {
		...
		// 
		// try to get file date by file name
		$ft = ibr_batch_datefromfilename($batch_file);
		$f_mtime = ($ft) ? $ft : date ("Ymd", filemtime($ar_batch['path']));
		//
	} else {
...
function ibr_batch_process_new ($file_array=NULL, $html_out=TRUE) { 
	...
	$p = csv_parameters("batch");
	$b_dir = dirname(__FILE__).$p['directory'];	
	//
	if($html_out) {
