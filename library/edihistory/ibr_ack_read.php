...
function ibr_ack_process_new($ack_files, $html_out = TRUE ) {
	...
	if ( is_array($ack_files) ) {
		//
		foreach($ack_files as $ack) { 
			$fp = '';
			$ext = strtolower(substr($ack, -3));
			$fp = csv_verify_file($ack, $ext); 
			if ($fp) {
				if ($ext == 'ack') { $ar_d[] = ibr_process_ack($fp); }
				if ($ext == 'ta1') { $ar_d[] = ibr_process_ta1($fp); }
			} else {
				$str_html .= "failed to verify file: $ack <br />" .PHP_EOL;
				csv_edihist_log("ibr_ack_process_new: failed to verify file: $ack");
				continue;
			}
		}
	} elseif ( is_string($ack_files) && strlen($ack_files) ) {
		$ext = strtolower(substr($ack_files, -3));
		$fp = csv_verify_file($ack_files, $ext);
		if ($fp) {
			if ($ext == 'ack') { $ar_d = ibr_process_ack($fp); }
			if ($ext == 'ta1') { $ar_d = ibr_process_ta1($fp); }
		} else {
			$str_html .= "failed to verify file: $ack_files <br />" .PHP_EOL;
			csv_edihist_log("ibr_ack_process_new: failed to verify file: $ack_files");
		}
	} else {
		// ack_files wasn't supplied, check for new files
		$new_ack = csv_newfile_list("ack");
		$new_ta1 = csv_newfile_list("ta1");
		$new_ack = array_merge( $new_ack, $new_ta1 );
		//		
	...
	if ( is_array($ar_d) && count($ar_d) ) {
		...
		if ($html_out) { 
			...
			$str_html .= "ACK/TA1 files: processed $fcount ACK/TA1 files <br />".PHP_EOL;
		}
	} else {
		$str_html .= "No new ACK/TA1 files found. <br />" .PHP_EOL;
	}
	//
	return $str_html;
}
