...
// constant -- unzip dir -- the directory into which uploads are stored
// if (!defined("IBR_UNZIP_DIR")) define("IBR_UNZIP_DIR", "/tmp/edizip");
// if (!defined("IBR_UPLOAD_DIR")) define("IBR_UPLOAD_DIR", "/tmp/editemp");
// $GLOBALS['site_dir'] $site = $GLOBALS['site_dir'] . "/edi/history/"
//if (!defined("IBR_HISTORY_DIR")) define("IBR_HISTORY_DIR", "/sites/default/edi/history");
	...
	foreach ($p as $ky=>$par) { 
		...
		if (!preg_match($p[$ky]['regex'], $fn) ) { 
			...
			if (preg_match('/[^\x20-\x7E\x0A\x0D]|(<\?)|(<%)|(<asp)|(<ASP)|(#!)|(\$\{)|(<scr)|(<SCR)/', $fstr, $matches, PREG_OFFSET_CAPTURE)) { 
				...
				//$newname = IBR_UPLOAD_DIR .DIRECTORY_SEPARATOR. $fn;
				...
				if (!$is_mv) {
					...
					if ( !is_writable( $ibr_upldir ) ) { 
						//echo "write permission denied for ".IBR_UPLOAD_DIR. "<br />" . PHP_EOL;
					...
					if (!$iscm) { 
						$html_str .= "Error: failed to set permissions for {$fa['name']}<br />" . PHP_EOL;
						unlink($newname);
						return FALSE;
					}
				}
			...
			break;
		} // end if (preg_match($ext_ar[$k][0], $fa	
	}
	return $ar_fn;
} 
...
function ibr_ziptoarray($zipfilename, $param_ar, &$html_str) {
    ...
    for ($i=0; $i<$f_ct; $i++) {
		...
		if ($fstr) { 
			...
			// rename the file, all in the IBR_UPLOAD_DIR
			// Note that BCBS files have no extension, just a name scheme (recently changed)			
			if (strpos($bnm, "835") === 0) { 
				//$newname = IBR_UPLOAD_DIR.DIRECTORY_SEPARATOR.$bnm."ediz";
                $newname = $ibr_upldir.DIRECTORY_SEPARATOR.$bnm."ediz";
			} else {
				//$newname = IBR_UPLOAD_DIR.DIRECTORY_SEPARATOR."ediz".$bnm;
			...
			if($newCrc !== $oldCrc && ($oldCrc + 4294967296) !== $newCrc) { 
				...
				$fzp['name'] = $bnm;
				$fzp['tmp_name'] = $newname;		// IBR_UPLOAD_DIR/ediz.$bnm or 83511111---ediz
...
function ibr_upload_files(&$html_str) {
	...
	//$fkey = array_key_exists("fileUplZIP", $_FILES) ? "fileUplZIP" : $fkey; // this is redundant since fileUplMulti does zip files
	...
	foreach($files[$fkey] as $idx=>$fa) {
        ...
        //$file_name = trim(basename(stripslashes($fa['name'])), ".\x00..\x20");
		...
		if ( $fkey == "fileUplMulti" && strpos($fa['name'], ".zip") ) {
			...
			if (is_array($f_upl) && count($f_upl)) { 
				foreach($f_upl as $tp=>$fz) {
					...
					if ( strlen($tp) && strpos("|batch|ibr|ebr|dpr|f997|f277|era|ack|ta1|text", $tp) ) {
	...
	if ( count($files_array) > 0 ) {
		...
		foreach ($files_array as $key=>$val) {
			...
			if ( strpos("|batch|ibr|ebr|dpr|f997|f277|era|ack|ta1|text", $key) ) {
				//$t_dir = dirname(__FILE__).$p_ar[$key]['directory'];
			...
			foreach($files_array[$key] as $idx=>$nf) {
				...
				if ( is_file($testname) ) {
					...
					if (!$iscm) { 
						...
						$prc_htm .= "Error: failed to set permissions for $nfb, removing file<br />" . PHP_EOL;
						unlink($testname);
					}					
			...
			if (count($files_array[$key]) == 0) { 
				$prc_htm .= "ibr_sort_upload: no new [ $key ] files in upload <br />" . PHP_EOL;
				continue; 
			}
		}
	} else {
