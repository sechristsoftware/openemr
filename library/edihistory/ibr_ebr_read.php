...
/** 
 * another rendition of a parser function for patient control number
 * 
 * @deprecated
 * @param string
 * @return array
 */
function ibr_pid_encounter_parse( $pid_enctr ) {
    ...
    // returning pidencounter, check value and use IBR_ENCOUNTER_DIGIT_LENGTH constant 
    $inv_split = preg_split('/\D/', $pid_enctr, 2, PREG_SPLIT_NO_EMPTY);
    // question: would it be better to just check for a separator?
    //
 ...
 * @param array  the array from ibr_ebr_filetoarray()
...
function ibr_dpr_html ($ar_dpr_data, $err_only=FALSE) {
	// @param array $ar_dpr_data -- data array produced by ibr_dpr_values()
	//  @param bool $err_only -- whether to outputonly in case of rejected claims
	//
	$hasclm = FALSE;
	$str_html = "";
	$idx = 0;
...
function ibr_ebr_message($ebrfile, $clm01, $batchnm = '') {
	...
	if ($fp) {
		...
		csv_edihist_log("ibr_ebr_message: file read error $dpr_filepath");
	...
	foreach($ebr_ar as $fld) {
		...
		//
		if ($isfound) {
			// there should be only one of these three possibilities
			if ($fld[0] == '3e') {
				//3e│Error Initiator│R│Error Code – if available, otherwise NA│Error Message | Loop│Segment ID│Element # ││││Version |
				// different for 4010, one less field, so loop gives segment 
...
function ibr_ebr_values($file_path) { 
	...
	foreach($ar_ebr as $ln) {
		...
		if (strval($ln[0]) == '3c') { 
			...
			//$ar_val[$b]['claims'][$c]['3c']['err_init'] = $ln[1]; 		// Error Initiator (usually payer)
			//$ar_val[$b]['claims'][$c]['3c']['err_type'] = $ln[2]; 		// message type
			//$ar_val[$b]['claims'][$c]['3c']['err_code'] = $ln[3]; 		// Error Code or NA
			//$ar_val[$b]['claims'][$c]['3c']['err_msg'] = $ln[4];			// message
			//$ar_val[$b]['claims'][$c]['3c']['err_loop'] = $ln[5]; 		// Loop or NA
			//$ar_val[$b]['claims'][$c]['3c']['err_seg'] = $ln[6]; 		// Segment ID or NA
			//$ar_val[$b]['claims'][$c]['3c']['err_elem'] = $ln[7];		// element or NA
			//
			continue;
		}
		//
...
function ibr_ebr_process_new_files($files_ar=NULL, $extension='ibr', $html_out=TRUE, $err_only=TRUE ) { 
	...
	$chrc = 0;
	//
	echo "ibr_ebr_process_new_files: " . count($f_list) . "<br />".PHP_EOL;
	//
	// sort ascending so latest files are last to be output
	$is_sort = asort($f_list);  // returns true on success
	//
