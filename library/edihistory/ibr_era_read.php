 ...
 */  
  ...
  * @return array               array('pid' => $p, 'enctr' => $e)  
  */
  function ibr_era_pid_enctr_parse( $pid_enctr ) {
	  ...
	  $inv_split = preg_split('/\D/', $pid_enctr, 2, PREG_SPLIT_NO_EMPTY);
	  // debug
	  //echo "Preg Split" . PHP_EOL;
	  //var_dump ($inv_split);
	  //
	  if (count($inv_split) < 2) {
		  $ct = strlen($pid_enctr);
		  $epos = $ct - IBR_ENCOUNTER_DIGIT_LENGTH;
		  $p = substr($pid_enctr, 0, $epos);
		  $e = substr($pid_enctr, $epos);
		  $acct = array('pid' => $p, 'enctr' => $e);
		  // debug
		  //echo "Un-mangled pid-encounter: $p $e" . PHP_EOL;
	  } else {
		  //$acct = array('pid' => $inv_split[0][0], 'enctr' => $inv_split[1][0]);
		  $acct = array('pid' => $inv_split[0], 'enctr' => $inv_split[1]);
	  }
	  return $acct;
  }   
 ...
 function ibr_era_claim_vals ( $ar_st_slice, $elem_delim, $comp_delim ) { 
	 ...
	 foreach ( $ar_st_slice as $segtext ) {
		 ...
		 if ($seg[0] == "CLP" ) { 
			 ...
			 // ibr_era_pid_enctr_parse( $pid_enctr )
			 // $inv_split = preg_split('/\D/', $seg[1], -1, PREG_SPLIT_NO_EMPTY);
 ...
 */		
 function ibr_era_html_page ( $file_path, $trn_trace=0, $pid_enctr=0, $searchtype='ALL', $fname='835 Remittance Advice') {
	 ...
	 //
	 if (!$f_path) {
		 csv_edihist_log("ibr_era_html_page: failed verification $f_path");
		 return "ibr_era_html_page: failed verification for $f_path <br />" . PHP_EOL;
	 } else {
		 $fn = basename($f_path);
		 $ar_era_segments = csv_x12_segments($f_path, "era", FALSE);
		 if (!is_array($ar_era_segments) || !count($ar_era_segments['segments']) >0 ) {
 ...
 */
 function ibr_era_csv_file_data ( $era_segments ) { 
	 ...
	 foreach($ar_era_segments as $segtxt) {
		...
		//
		if ($seg[0] == "TRN") {
			$trn_trace = trim($seg[2]); 	// trace number/check number -- required element
			continue;
		}
		//
		if ($seg[0] == "N1" && $seg[1] == "PR") { 
...
function ibr_era_csv_files_html ($trn_csv_array, $err_only=TRUE  ) {
	//
	// file summary table heading -- file output is per TRN transaction
	$htm_hdrf .= "<table class=\"eracsv\" cols=6><caption>ERA Files Summary CSV</caption>
       <thead>
         <tr>
    ...
    //   the  </tbody> is inserted to close the preceding file table
    $htm_hdrc .= "</tbody>
	  <table class=\"eracsv\" cols=6>
		<thead>
	      <tr>
	 ...
	 // $trn_csv_array structure is [S]['file']  [S]['claim'][C]  
	 //   so one 'file' and multiple 'claim' per ST-SE block
	 foreach($trn_csv_array as $ardata) {
		$idx = 0;
		$idxc = 0;
		...
		if (array_key_exists('file', $ardata) && count($ardata['file']) > 0 ) {
			...
			//
			// if a denied claim occurred in the preceding iteration, we will need to insert
			// the table heading
			if ($haserr) { 
		...
		if (array_key_exists('claim', $ardata) && count($ardata['claim']) > 0 ) {
			 ...
			 foreach ($ardata['claim'] as $clm) {		
				 ...
				 if ($err_only && ($clm['status'] != "4" && $clm['status'] != "22" && $clm['status'] != "23") ) { 
					 if (intval($clm['pmt']) > 0) continue;
				 }
				 //
				 $haserr++;
				 $bgclm = ($idxc % 2 == 1 ) ? 'odd' : 'even'; 
				 $idxc++;
		 ...
		 // we have run through each array in the set 'file' and 'claim'
		 //
		 $out_html .= $file_html;
...
/**
 * look in the files.csv table and find the trace and associated filename
 * 
 * @param string $trace   the trace or check number for a transaction
 * @return array          array([0]=> array(file, trace) )
 */ 
 function ibr_era_find_file_with_trace( $trace ) { 
	 // 
	 // get the file containing the control_num (expect only one match)
	 $era_file = FALSE;
	 //array('Date', 'FileName', 'Trace', 'claim_ct', 'Denied', 'Payer');
	 //
	 //$search_ar should have keys ['s_val']['s_col'] array(['r_cols'])
	 $sa = array('s_val'=>$trace, 's_col'=>'2', 'r_cols'=>array(1, 2)); 
	 $era_file = csv_search_record('era', 'file', $sa, "1");
	 //
	 if (!$era_file) {
		csv_edihist_log("ibr_era_find_file_with_trace: not found $trace"); 
	 }
	// $era_file should be like array([0]=> array(file, trace) )
	 return $era_file;
}	 
...
/**
 * look in the claims_era.csv table and find the pid and associated encounter and filename
 * 
 * This function enables us to identify all RA's for a particular patient ID
 * 
 * @deprecated
 * @param string $pid        the patient ID number
 * @return array             pid-enc, trace, and filename, possibly more than one
 */
 function ibr_era_find_file_with_pid ( $pid ) { 
	 // look in the csv claims table and find the pid and associated encounter and filename
	 // return the pid, encounter, and filename in an array
	 ...
	 if ($pval) {
		 $bpos = strpos($pval, "-");
		 $pd = ($bpos === false) ? $pval : substr($pval, 0, $bpos);
		 $slen = strlen($pd);
	 } else {
		 csv_edihist_log("ibr_era_find_file_with_pid: invalid pid $pid");
		 return false;
	 }
	 //array('PtName', 'SvcDate', 'clm01', 'Status', 'trace', 'claimID', 'File_835', 'Pmt', 'PtResp', 'Payer');
	 //
	 $params = csv_parameters('era');
	 $fp = dirname(__FILE__).$params['claims_csv'];
	 if (($fh = fopen($fp, "r")) !== FALSE) {
		 while (($data = fgetcsv($fh1, 1024, ",")) !== FALSE) {
			 // check for a match
			 if (substr($data[2], 0, $slen) == $pd) { 
				 $ret_ar[] = array($data[2], $data[4], $data[5]);
			 }
		 }
		 fclose($fh1);
 ...
 */ 
 function ibr_era_process_new ($file_array=NULL, $html_out=TRUE, $err_only=TRUE) { 
	 ...
	 foreach ($ar_newfiles as $f_era) {
		 //
		 $full_path = csv_verify_file($f_era, 'era');
		 // get the file data for csv output
		 //$ar_csv_data = ibr_era_csv_file_data ( $full_path );
		 // use csv_x12_segments() from csv_record_include.php
		 $era_segments = csv_x12_segments($full_path, "era", FALSE);
		 //
		 if (is_array($era_segments) && count($era_segments['segments']) > 0 ) {
			 ...
			 $htm_hdrc .= "<p>did not get segments for " . basename($full_path) . "</p>" . PHP_EOL;
			 continue;
		 }
	 }
	 ...
	 if ($html_out) {
		 foreach($ar_csv_data as $eradata) {
			 $html_str .= ibr_era_csv_files_html($eradata);
		 }
		 //
