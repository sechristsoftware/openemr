...
function escape_sql_column_name($s,$tables,$long=FALSE) {
      ...
      // Now can ecape the sql column name
...
function escape_identifier($s,$whitelist_items,$die_if_no_match=FALSE) {
      if (is_array($whitelist_items)) {
            ...
            if ( $die_if_no_match && !(in_array($s,$whitelist_items)) ) {
                  ...
                  error_Log("OpenEMR SQL Escaping ERROR of the following string: ".$s,0);
                  die(xlt("There was an OpenEMR SQL Escaping ERROR of the following string")." ".text($s));
            }
            $ok = $whitelist_items;
