 ...
 * @param string/array $currentPwd      the active user's current password (if in array form, then has already been hashed on client end)
 * @param string/array $newPwd          the new password for the target user (if in array form, then has aleady been hashed on client end)
...
{
    $userSQL="SELECT ".implode(",",array(COL_PWD,COL_SALT,COL_PWD_H1,COL_SALT_H1,COL_PWD_H2,COL_SALT_H2,COL_SALT_CLIENT))
    ...
    if($activeUser==$targetUser)
    {
        ...
        if (is_array($currentPwd)) {
            // Has already been hashed on client end
            $hash_current_client = $currentPwd['phash_client'];
        }
        else {
            // Password is in clear text (via rsa), so need to mimick the client end hash
            $hash_current_client = password_hash($currentPwd,$userInfo[COL_SALT_CLIENT]);
        }

        $hash_current = password_hash($hash_current_client,$userInfo[COL_SALT]);
        ...
        $adminSQL=" SELECT ".implode(",",array(COL_PWD,COL_SALT,COL_SALT_CLIENT))
                  ." FROM ".TBL_USERS_SECURE
                  ." WHERE ".COL_ID."=?";
        ...
        if (is_array($currentPwd)) {
            ...
            // Password is in clear text, so need to mimick the client end hash
            $hash_current_client = password_hash($currentPwd,$adminInfo[COL_SALT_CLIENT]);
        }

        $hash_admin = password_hash($hash_current_client,$adminInfo[COL_SALT]);
    ...
    if (!is_array($newPwd)) {
        // NOTE can only do these checks when using cleartext password, so will
        // also need to check these on the client end!!
        if(strlen($newPwd)==0)
        {
    ...
    if($userInfo===false)
    {
        // No userInfo means either a new user, or an existing user who has not been migrated to salt mechanism yet
        ...
        if($create)
        {
            ...
            $getUserID=  " SELECT ".COL_ID
                ...
                // Note that below function will automatically manage whether password is in clear text or has been hashed on client.
    ...
    else
    {
        ...
        if($forbid_reuse)
        {
            ...
            if (is_array($newPwd)) {
                // Client side hash already performed
                $hash_current = password_hash($newPwd['phash_client'],$userInfo[COL_SALT]);
                $hash_history1 = password_hash($newPwd['phash_client'],$userInfo[COL_SALT_H1]);
                $hash_history2 = password_hash($newPwd['phash_client'],$userInfo[COL_SALT_H2]);
            }
            else {
                // Client side hash not yet performed (passwords are in clear text via rsa)
                $hash_client = password_hash($newPwd,$userInfo[COL_SALT_CLIENT]);
                $hash_current = password_hash($hash_client,$userInfo[COL_SALT]);
                $hash_history1 = password_hash($hash_client,$userInfo[COL_SALT_H1]);
                $hash_history2 = password_hash($hash_client,$userInfo[COL_SALT_H2]);
            }
        ...
        if (is_array($newPwd)) {
            // Client side hash already performed
            $newHash_client = $newPwd['phash_client'];
        }
        else {
            // Client side hash not yet performed (passwords are in clear text via rsa)
            $newHash_client =  password_hash($newPwd,$userInfo[COL_SALT_CLIENT]);
        }

        $newSalt = password_salt();
        $newHash = password_hash($newHash_client,$newSalt);
        $updateParams=array();
