 ...
 *   initial_rxcui     -> rxcui number of initital term type
 *   initial_name      -> drug name of initial term type
 * </pre>
 *
...
function rxcui_resolve_rxnorm($rxcui,$term_type,$drug_name) {
  ...
  if ($term_type == "IN") {
    ...
    $sql = "SELECT conso.RXCUI as rxcui, conso.STR as full_name, conso.TTY as term_type, " .
           "'" . add_escape_custom($term_type) . "' as initial_term_type, " .
           "'" . add_escape_custom($rxcui) . "' as initial_rxcui, " .
           "'" . add_escape_custom($drug_name) . "' as initial_name " .
           "FROM `RXNCONSO` as conso " .
           "JOIN `RXNREL` as rel ON conso.RXCUI = rel.RXCUI1 " .
...
/**
 * Checks if can use rxnorm sql tables for new prescriptions.
 *
 * @return  boolean  TRUE if can use rxnorm sql tables, FALSE otherwise.
 */
function check_rxnorm_new_rx() {
  if ( $GLOBALS['rx_enable_rxnorm'] && check_rxnorm_database_exist() ) {
    return TRUE;
  }
  else {
    return FALSE;
...
/**
 * Checks if rxnorm sql tables exists.
 *
 * @return  boolean  TRUE if rxnorm sql tables exist, FALSE otherwise.
 */
function check_rxnorm_database_exist() {
