...
// --------- check if reminder is due 
            ...
            $pdHTML .= '<p id="p_'.attr($r['messageID']).'">
                          ...
                          <span title="'.xl('Click Patient Name to Open Patient').'" class="'.attr($class).'">'. 
                            $warning.' 
                            <span onclick="goToPatient('.attr($r['PatientID']).')" class="patLink" id="'.attr($r['PatientID']).'">'.
                              text($r['PatientName']).'
                            </span> '.
                            text($r['message']).' - ['.text($r['fromName']).']
                          </span> -----> 
                          <a onclick="openAddScreen('.attr($r['messageID']).')" class="dnForwarder" id="'.attr($r['messageID']).'" href="#">[ '.xl('Forward').' ]</a>
                        </p>';
          }
      return ($pdHTML == '' ? '<p class="alert"><br />'.xlt('No Messages').'</p>' : $pdHTML); 
    } 
             ...
             $.post("<?php echo $GLOBALS['webroot']; ?>/interface/main/dated_reminders/dated_reminders.php", { drR: id }, 
               function(data) {
                if(data == 'error'){     
                  ...
                  if(id > 0){
                    $(".drTD").html('<p style="text-size:3em; margin-left:200px; color:black; font-weight:bold;"><?php echo xla("Refreshing Messages") ?> ...</p>');
                  }
                  $(".drTD").html(data); 
                }   
