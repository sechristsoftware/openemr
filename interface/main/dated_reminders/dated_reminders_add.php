    ...
    <script language="JavaScript">   
      $(document).ready(function (){  
        $('#selectPatient').click(function(){ 
          // todo : get patient selector working
        ...
        $("#sendButton").click(function(){
          ...
          if(noRecipient){
             errorMessage = errorMessage + '* <?php echo addslashes(xl('Please Select A Recipient')) ?><br />';
          }  
          ...
          if(dueDate == ''){
             errorMessage = errorMessage + '* <?php echo addslashes(xl('Please enter a due date')) ?><br />';
          }        
          
          // check if message is set                                   
          if(message == ''){
             errorMessage = errorMessage + '* <?php echo addslashes(xl('Please enter a message')) ?><br />';
          }  
...
<div id="overDiv" style="position:absolute; visibility:hidden; z-index:1000;"></div>
    ...
    <form id="addDR" style="margin-left:40px;" id="newMessage" method="post">
     <div style="text-size:2em; text-align:center; color:red" id="errorMessage"></div>                                                
       <?php echo xlt('Link to Patient'),'(',xlt('optional'),')<a href="#" id="selectPatient"> ',xlt('click to select'),'</a>'; ?>
       <span id="patientIDSpan">
        <input name="PatientID" id="PatientID" type="hidden" value="<?php echo (isset($pid) ? attr($pid) : 0) ?>" />
        <input style="width:200px;" disabled="disabled" id="patientName" value="<?php echo attr(getPatName()) ?>" />
        <a href="#" id="removePatient">[<?php echo xlt('unlink patient') ?>]</a>
       </span>
     <br /><br />  
     
        
     <?php echo xlt('Send to') ?> :  <p style="line-height:1.8em;">
               <input class="sendTo" type="checkbox" name="sendTo_me" value="<?php echo xla(intval(attr($_SESSION['authId']))) ?>" id="me"><label for="me"><?php echo xlt('Myself') ?></label>&nbsp;&nbsp;&nbsp;&nbsp; 
                <?php //     
      ...
      <?php echo xlt('Due Date') ?> : <input type='text' name='dueDate' id="dueDate" size='20' value="<?php echo ($this_message['dueDate'] == '' ? date('Y-m-d') : xla($this_message['dueDate'], ENT_QUOTES)); ?>" onkeyup='datekeyup(this,mypcc)' onblur='dateblur(this,mypcc)' title='<?php echo htmlspecialchars( xl('yyyy-mm-dd'), ENT_QUOTES); ?>' />      
      <?php echo xlt('OR') ?> 
      ...
      <?php echo xlt('Priority') ?> : 
                 <input <?php echo ($this_message['priority'] == 3 ? 'checked="checked"' : '') ?> type="radio" id="priority" name="priority" value='3'> <?php echo xlt('Low') ?> 
                 <input <?php echo ($this_message['priority'] == 2 ? 'checked="checked"' : '') ?>type="radio" name="priority" id="priority"  value='2'> <?php echo xlt('Medium') ?>  
                 <input <?php echo ($this_message['priority'] == 1 ? 'checked="checked"' : '') ?>type="radio" name="priority" id="priority"  value='1'> <?php echo xlt('High') ?>
      <br />         
      <br />
