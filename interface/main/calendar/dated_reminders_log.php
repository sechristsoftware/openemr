  ...
  if($_GET){ 
    echo '<table border="1" width="100%" cellpadding="5px" id="logTable">
            <thead>
              <tr>
                ...
                <th>'.xl('Message').'</th>
                <th>'.xl('Due Date').'</th>
                <th>'.xl('Proccessed Date').'</th>
                <th>'.xl('Proccessed By').'</th>
              </tr>
            </thead>
            <tbody>';
    ...
    foreach($remindersArray as $RA){ 
      echo '<tr class="heading">
              ...
              <td>',$RA['fromName'],'</td>
              <td>',$RA['ToName'],'</td>
              <td>',$RA['PatientName'],'</td>     
              <td>',$RA['message'],'</td>    
              <td>',$RA['dDate'],'</td>    
              <td>',$RA['pDate'],'</td>      
              <td>',$RA['processedByName'],'</td>
            </tr>';
    }
    echo '</tbody></table>'; 
...
?>     
    <form method="get" id="logForm">         
      <h1><?php echo xl('Dated Message Log') ?></h1>  
      <h2><?php echo xl('filters') ?> :</h2>
      <blockquote><?php echo xl('Date The Message Was Sent') ?><br />
<!----------------------------------------------------------------------------------------------------------------------------------------------------->  
      <?php echo xl('Start Date') ?> : <input id="sd" type="text" name="sd" value="" onkeyup='datekeyup(this,mypcc)' onblur='dateblur(this,mypcc)' title='<?php echo htmlspecialchars( xl('yyyy-mm-dd'), ENT_QUOTES); ?>' />   &nbsp;&nbsp;&nbsp;  
<!----------------------------------------------------------------------------------------------------------------------------------------------------->   
      <?php echo xl('End Date') ?> : <input id="ed" type="text" name="ed" value="" onkeyup='datekeyup(this,mypcc)' onblur='dateblur(this,mypcc)' title='<?php echo htmlspecialchars( xl('yyyy-mm-dd'), ENT_QUOTES); ?>' />   <br /><br />
<!----------------------------------------------------------------------------------------------------------------------------------------------------->   
      </blockquote>
      <p style="line-height:1.8em;">       
        <?php echo xl('Sent By') ?> :                                     
        <input type="checkbox" id="sentBy_all"><label for="sentBy_all"><?php echo xl('Select All') ?></label><br />
        <input class="sentBy" type="checkbox" name="sentBy_me" value="<?php echo intval($_SESSION['authId']) ?>" id="sentBy_me"><label for="sentBy_me">Me</label>&nbsp;&nbsp;&nbsp;&nbsp;   
        <?php //  
            $i = 2;   
            foreach($allUsers as $user){
              echo '<input class="sentBy" type="checkbox" name="sentBy_',$i,'" id="sentBy_',$i,'" value="',$user['id'],'"><label for="sentBy_',$i,'">',$user['fname'],' ',$user['mname'],' ',$user['lname'],'</label>&nbsp;&nbsp;&nbsp;&nbsp; ';
...
<!----------------------------------------------------------------------------------------------------------------------------------------------------->     
      <p style="line-height:1.8em;">  
      Sent To :     
        <input type="checkbox" id="sentTo_all"><label for="sentTo_all"><?php echo xl('Select All') ?></label><br />
        <input class="sentTo" type="checkbox" name="sentTo_me" value="<?php echo intval($_SESSION['authId']) ?>" id="sentTo_me"><label for="sentTo_me">Me</label>&nbsp;&nbsp;&nbsp;&nbsp;   
        <?php //  
            $i = 2;   
            foreach($allUsers as $user){
              echo '<input class="sentTo" type="checkbox" name="sentTo_',$i,'" id="sentTo_',$i,'" value="',$user['id'],'"><label for="sentTo_',$i,'">',$user['fname'],' ',$user['mname'],' ',$user['lname'],'</label>&nbsp;&nbsp;&nbsp;&nbsp; ';
              // line break for every 4 users
              if($i % 4 == 0) echo "<br />";  
              $i++; 
            }
...
<!-----------------------------------------------------------------------------------------------------------------------------------------------------> 
      <input type="checkbox" name="processed" id="processed"><label for="processed"><?php echo xl('Processed') ?></label>      
<!-----------------------------------------------------------------------------------------------------------------------------------------------------> 
      <input type="checkbox" name="pending" id="pending"><label for="pending"><?php echo xl('Pending') ?></label>          
<!----------------------------------------------------------------------------------------------------------------------------------------------------->  
      <br /><br />  
      <button value="Refresh" id="submitForm"><?php echo xl('Refresh') ?></button>
    </form>
    
    <div id="resultsDiv"></div> 
<?php      
  }else{
    echo "Permisions Error.";
  }
?>   
  </body> 
...
// -------------------------------------  if there was a patient linked, set the name, else set it to blank          
          ...
          $reminders[$i]['PatientName'] =  (empty($pRow) ? 'N/A' : ucwords(strtolower($reminders[$i]['PatientName'])));
// -------------------------------------
          ...
          $reminders[$i]['ToName'] =  ucwords(strtolower($reminders[$i]['ToName']));            
          $reminders[$i]['processedByName'] = (empty($prRow) ? '' : $prRow['ptitle'].' '.$prRow['pfname'].' '.$prRow['pmname'].' '.$prRow['plname']); 
          $reminders[$i]['processedByName'] =  (empty($prRow) ? 'N/A' : ucwords(strtolower($reminders[$i]['processedByName'])));  
      	} 
