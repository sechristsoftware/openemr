...
>>>>>>> f0e881d... Mixed modification from Alex, take 2. (based on rel-410, patch 8 code)
  echo " </tr>\n";
}
...
=======
    ...
    while ($irow = sqlFetchArray($ires)) {
      ...
      $patient_id = $irow['pid'];
      $enc = $irow['encounter'];
      $totalsales = $irow['sales']; //ALB This will be drug sales and taxes

      //ALB Need to add taxes for drug sales here
      $dquery = "SELECT s.drug_id, s.fee FROM drug_sales " .
        "as s WHERE s.pid = '$patient_id' AND " .
        "s.encounter = '$enc'"; 
      $dres = sqlStatement($query);
      while ($drow = sqlFetchArray($dres)) {
         // Add taxes.
        $trow = sqlQuery("SELECT taxrates FROM " .
          "drug_templates WHERE drug_id = '" .
          $drow['drug_id'] . "' ORDER BY selector LIMIT 1");
        $totalsales += calcTaxes($trow, $drow['fee']);
      }

      $balance = $irow['charges'] + $totalsales + $irow['copays'] - $irow['ptpaid'] - $irow['inspaid'] - $irow['adjustments'];
     ...
     if (is_null($irow['charges']) && is_null($irow['sales'])) { 
     ...
     if ((!$balance || bucks(abs($balance)) == 0.00) && substr($irow['date'], 0, 10) != $today) continue;
      ...
      if (substr($irow['date'], 0, 10) == $today) {
        ...
        //ALB This will output the lines for today that will then go into the billing table once the form is saved.
        echoLine("form_upay[$enc]", $svcdate, $irow['charges'] + $totalsales, $ptpaid, $inspaid, $duept);
      } else {
        //ALB This will output the lines for previous visits that will go into the ar_activity table once the form is saved.
        echoLine("form_bpay[$enc]", $svcdate, $irow['charges'] + $totalsales, $ptpaid, $inspaid, $duept);
      }
    }
  } // end $INTEGRATED_AR
>>>>>>> f0e881d... Mixed modification from Alex, take 2. (based on rel-410, patch 8 code)
