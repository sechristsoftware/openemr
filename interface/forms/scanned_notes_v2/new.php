...
function addNewScannedFormDocument($file,$name,$encounter,$formid,$patient_id,$path,$path_depth='1',$category='1') {

    $TEMP_FILES = array();
    $TEMP_FILES['file']['name'][0]=$name;
    $TEMP_FILES['file']['type'][0]=$file['form_image']['type'];
    $TEMP_FILES['file']['tmp_name'][0]=$file['form_image']['tmp_name'];
    $TEMP_FILES['file']['error'][0]=$file['form_image']['error'];
    $TEMP_FILES['file']['size'][0]=$file['form_image']['size'];
    $_FILES = $TEMP_FILES;

    $_GET['higher_level_path']=$path;
    $_GET['patient_id']=$patient_id;
    $_POST['destination']='';
    $_POST['submit']='Upload';
    $_POST['path_depth']=2;
    $_POST['patient_id']=$patient_id;
    $_POST['category_id']=$category;
    $_POST['process']='true';
...
if ($_POST['bn_save']) {
 ...
 // If adding a new form...
  ...
  if ($_FILES['form_image']['size']) {
   ...
   // TODO, figure out the category id

   $document_id = addNewScannedFormDocument($_FILES,$encounter."_".$formid."_".$_FILES['form_image']['name'],$encounter,$formid,$pid,'scanned_encounter_notes/'.$pid,2,1);
   if ($document_id) {
    sqlStatement("UPDATE form_scanned_notes_v2 SET doc_id=? WHERE id=?", array($document_id,$formid) );
   }
  }
