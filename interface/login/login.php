...
function encrypt_form(user)
{
    ...
    // (If rsa is not available, it will support seamless migration of user to the new salt
    //  methodology)
    ...
    $.post(rsa_ajax,{user: user},
        function(data)
        {
            ...
            if (method == "rsa") {
                ...
                // If rsa is not supported (this is the backup methods)
                var salt = data.salt;
                ...
                // If rsa is not supported and user will be migrated to new salt method
                var salt = data.salt;
                var encryptedPass='$SHA1$' + SHA1(salt + document.forms[0].clearPass.value);
