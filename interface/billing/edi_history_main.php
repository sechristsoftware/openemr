...
/**
 * the directory to where uploaded files are moved 
 */
//if (!defined("IBR_UPLOAD_DIR")) define("IBR_UPLOAD_DIR", "/tmp/edihist");
/**
 * directory tree for uploaded files and csv record files
...
//if (!defined("IBR_FTYPES")) define("IBR_FTYPES", "|batch|ack|ta1|f997|f277|ibr|ebr|dpr|era|text");
//if (!defined("IBR_JS_DIR")) define("IBR_JS_DIR", "jscript/");
//if (!defined("IBR_CSS_DIR")) define("IBR_CSS_DIR", "jscript/style/");

//require dirname(__FILE__) . DIRECTORY_SEPARATOR . 'my_file.php';
// path will become "../library/edihist/filename.php"
//    "$srcdir/edihistory/filename.php
...
//header("Cache-Control: no-cache, must-revalidate, private"); // HTTP/1.1
//header("Expires: Sat, 26 Jul 1997 05:00:00 GMT"); 			 // Date in the past

if (isset($GLOBALS['OE_SITE_DIR'])) {
	//$GLOBALS['OE_EDIH_DIR'] will replace IBR_HISTORY_DIR
	//$GLOBALS['OE_EDIH_DIR'] = $GLOBALS['OE_SITE_DIR'].'/edi/history';
...
if (strtolower($_SERVER['REQUEST_METHOD']) == 'post') {
	...
	if (isset($_GET['srvinfo']) && $_GET['srvinfo'] == 'yes') { 
		...
		$html_str = "unknown GET Array parameter<br />" .PHP_EOL;
		$html_str .= var_dump($_GET) . PHP_EOL;
	}
	
} else {
	die("edi_history: invalid input method <br />");
}
