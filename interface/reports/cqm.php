 ...
 function runReport() {
   ...
   $.get("../../library/ajax/collect_new_report_id.php",
     function(data){
       ...
       // Run the report
       top.restoreSession();
       $.post("../../library/ajax/execute_report.php",
...
<?php if (!empty($report_id)) { ?>
  <?php echo " - " . xl('Date of Report') . ": " . $date_report;
						...
						<span>
							<?php echo htmlspecialchars( xl('Print'), ENT_NOQUOTES); ?>
                                            ...
                                            <?php if ($back_link == "list") { ?>
                                               <a href='cqm_list.php' class='css_button' onclick='top.restoreSession()'><span><?php echo xlt("Return To Report Results"); ?></span></a> 
                                            <?php } else { ?>
