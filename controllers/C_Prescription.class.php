...
class C_Prescription extends Controller {
    ...
    ;
        ...
        function multiprintcss_header($p) {
	        ...
	        echo ("<td>\n");
                ...
                if ($GLOBALS['show_DEA']) echo ('<span class="large"><b>' . xl('DEA') . ':</b>' . $p->provider->federal_drug_id . '</span><br>');
                else echo ('<b><span class="large">' . xl('DEA') . ':</span></b> ________________________<br>' );
                if ($GLOBALS['display_NPI'])
                if ($GLOBALS['show_NPI']) echo ('<span class="large"><b>' . xl('NPI') . ':</b>' . $p->provider->npi . '</span><br>');
				else echo ('<b><span class="large">' . xl('NPI') . ':</span></b> ________________________<br>');
                ;
               if ($GLOBALS['display_SLN'])
               if ($GLOBALS['show_SLN']) echo ('<span class="large"><b>' . xl('State Lic. #') . ':</b>' . $p->provider->state_license_number . '</span><br>');
	...
	function multiprint_body(& $pdf, $p){
		...
		$pdf->ezSetY($my_y);
		$pdf->ezText($d,10);
		$pdf->ez['leftMargin'] = $GLOBALS['left'];
		$pdf->ez['rightMargin'] = $GLOBALS['right'];
		$pdf->ezText('');
		$pdf->line($pdf->ez['leftMargin'],$pdf->y,$pdf->ez['pageWidth']-$pdf->ez['rightMargin'],$pdf->y);
		$pdf->ezText('');
	}
	...
	function multiprint_action($id = "") {
		...
		$pdf =& new Cezpdf($GLOBALS['paper_size']);
		$pdf->ezSetMargins($GLOBALS['top']
			,$GLOBALS['bottom']
			,$GLOBALS['left']
			,$GLOBALS['right']
		);
